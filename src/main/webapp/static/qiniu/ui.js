/*global plupload */
/*global qiniu */
function FileProgress(up,file,targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        progressCancel.addClass('progressCancel').text('');
        ...
        var progressOpTd = $("<td/>");
        var opDiv = $("<div id='opBox-"+file.id+"' />");
        var opSpan = $("<span id='opDel-"+file.id+"' />");
        var delLink = $("<a />");
        delLink.text("删除");
        delLink.on('click', function() {
        	removeSpecFile (up,file.id);
        });
        opSpan.append(delLink);
        opDiv.append(opSpan);
        progressOpTd.append(opDiv);

        Wrappeer.append(progressText);
        Wrappeer.append(progressSize);
        Wrappeer.append(progressBarTd);
        Wrappeer.append(progressOpTd);
...
//删除文件
function removeSpecFile (upObj,fileId) {
	var attaArrIdx = new Array();
	//console.info("files count before: "+upObj.files.length);
	upObj.removeFile(upObj.getFile(fileId));
	$("#"+fileId).remove();
	...
	if(attaFile != null){
		for(var i=0;i<attaFile.length;i++){
			...
			if(attaObj.fileId == fileId){
				attaArrIdx.push(i);//添加要删除的下标
			}
		}
	}
	for(var j=0;j<attaArrIdx.length;j++){
		attaFile.remove(attaArrIdx[j]);
	}

	//console.info("files count after: "+upObj.files.length);
};
